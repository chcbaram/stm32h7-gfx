...
**`.FLM` 인터페이스로는 손댈 수 없다.** `Init(adr, clk, fnc)` 에 병렬도를 알릴
...
> **덧붙임 —** 이 문단은 처음에 "표준 인터페이스로는 손댈 수 없다" 라고 썼는데
> 범위를 잘못 잡은 것이었다. `.FLM` 에는 통로가 없지만 **ST 의 `.stldr` 에는
> 있다.** 계획에 이미 적어둔 포맷을 후보로 세지 않은 탓이다.
> [10편](10-stldr.md)에서 이 격차를 닫고 ST 공식 도구보다 빨라진다.
...
- **ST 공식 도구보다 1.3배 느리다.** 그 차이는 전부 소거 병렬도에서 온다.
  ([10편](10-stldr.md)에서 뒤집힌다 — 1.1배 빠름)
...
[9. 펌웨어 파일 세 가지](09-image-format.md) — 굽는 것 자체는 되니 이제 쓰기 좋게
만들 차례다. `.bin` 말고 `.elf` 와 Intel HEX 까지 받는다.
