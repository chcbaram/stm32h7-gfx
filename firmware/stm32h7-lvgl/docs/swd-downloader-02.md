# STM32로 STM32를 굽는다 (2) — 플래시 알고리즘을 어디서 구하나

[1편](swd-downloader-01.md)에서 선 두 가닥으로 타깃 MCU를 세우고 메모리를 읽고 쓰는
데까지 갔다. 이제 실제로 **플래시를 구울 차례**인데, 여기서 근본적인 질문이 하나 나온다.
...
호출하는가" 하나뿐이고, 그건 [1편의 Stage 4](swd-downloader-01.md)에서 이미 만들었다.
...
**1편에서 SWD로 칩에서 직접 읽은 값과 정확히 일치한다.** DBGMCU가 알려준 DEV_ID
...
준비는 끝났다. 다음은 펌웨어 안에 **ELF32 파서**를 넣는 일이다.

- 섹션 헤더를 훑어 `PrgCode` / `PrgData`를 찾고
- 심볼 테이블에서 `Init` / `EraseSector` / `ProgramPage` 주소를 뽑고
- `DevDscr`에서 `FlashDevice`를 읽고
- 재배치해서 타깃 RAM에 올린다

파일을 통째로 메모리에 올리지 않고 `f_lseek`/`f_read`로 필요한 부분만 읽는 스트리밍
파서로 만들 계획이다. 13KB짜리는 통째로 읽어도 되지만, `.stldr`은 수백 KB짜리도
있어서 처음부터 스트리밍으로 짜두는 게 낫다.
