...
```
┌──────────────────────────────────────────────┐
│ swd_cm.c   Cortex-M 디버그 코어               │  halt / run / step
│                                              │  레지스터, 리셋
├──────────────────────────────────────────────┤
│ swd_dap.c  ARM ADIv5 DP/AP                   │  타깃 메모리 읽기·쓰기
├──────────────────────────────────────────────┤
│ swd.c      SWD 전송 (비트뱅잉)                 │  패킷, ACK, 패리티
└──────────────────────────────────────────────┘
                  PE3 / PC10
```

위 두 계층은 **ARM 표준**이라 벤더와 무관하다. ST든 Nordic이든 NXP든 같은 코드가
돈다. 이게 나중에 "다른 제조사 MCU 지원"이 거의 공짜가 되는 이유다.
...
다음은 **타깃 RAM에 코드를 올려 실행시키는 것**이다. 플래시를 굽는 알고리즘은
결국 타깃 자신의 RAM에서 돌아야 하기 때문이다. ARM의 CMSIS-Pack 표준 플래시
알고리즘(`.FLM`)과 ST의 External Loader(`.stldr`)를 SD카드에서 읽어 그대로
쓰는 것이 목표다.

재밌는 건, 그 앞에 검증 단계를 하나 두기로 한 것이다. ELF 파일을 건드리기 전에
**손으로 짠 4바이트짜리 기계어 세 개**로 실행 엔진 자체를 먼저 검증한다.

| 블롭 | 코드 | 확인하는 것 |
|---|---|---|
| `0xBE001840` | `adds r0,r0,r1` / `bkpt` | 레지스터 세팅 → 실행 → 정지 → 반환값 |
| `0x47701840` | `adds r0,r0,r1` / `bx lr` | 실제 알고리즘과 같은 복귀 경로 |
| `0xE7FEE7FE` | `b .` (무한루프) | 타임아웃과 강제 정지 |

A는 되는데 B가 안 되면 LR의 Thumb 비트 문제, 둘 다 안 되는데 레지스터는 정상이면
xPSR의 T 비트 문제 — 이렇게 원인이 바로 갈리도록 설계했다.
